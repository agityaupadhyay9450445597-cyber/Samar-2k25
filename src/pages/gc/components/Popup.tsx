...
const Popup: React.FC<PopupProps> = ({ game, onClose }) => {
    ...
    // 🚨 CORRECTED: Spread and sort the 'matches' array
    ...
    return (
        <div className="fixed inset-0 backdrop-blur-sm bg-black bg-opacity-5 flex items-center justify-center z-50">
            <div 
                ...
                style={{ backgroundImage: `url(${game.image})`, backgroundSize: "cover", backgroundPosition: "center", backgroundRepeat: "no-repeat" }}
            >
                <button
                    onClick={onClose}
                ...
                <div className="space-y-3 max-h-100 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200 pr-2">
                    {sortedMatches.map((match, index) => (
                        ...
                        >
                            ...
                            {match.team2Id ? ( 
                                <>
                                    {/* 🚨 CORRECTED: Use getTeamName utility for names */}
                                    <p>{getTeamName(match.team1Id)} VS {getTeamName(match.team2Id)}</p>
                                    <p>
                                        {/* 🚨 CORRECTED: Use winner ID directly */}
